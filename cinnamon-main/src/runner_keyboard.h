...
// ===[ Per-instance keyboard override ]===
//
// Lets a platform backend answer keyboard queries differently depending on which instance
// is asking. A game whose player object reads the keyboard directly then becomes
// multi-player without touching its code: each player instance simply hears its own
// controller. Left null, nothing changes and every query goes to the shared keyboard.
//
// `mode` distinguishes the three GML queries, and the hook returns false to decline and
// fall through to the normal path.
typedef enum {
    KEYBOARD_QUERY_CHECK = 0,
    KEYBOARD_QUERY_PRESSED,
    KEYBOARD_QUERY_RELEASED,
} KeyboardQueryMode;

struct Instance;
typedef bool (*KeyboardOverrideFn)(struct Instance* instance, int32_t key,
                                   KeyboardQueryMode mode, bool* outResult);

extern KeyboardOverrideFn gKeyboardOverride;
