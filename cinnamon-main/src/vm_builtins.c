...
// Keyboard functions
// The three keyboard queries first offer the instance that is asking to the platform's
// override hook, if one is installed. That is what lets a backend give different player
// instances different controllers without the game knowing; see runner_keyboard.h.
static RValue builtin_keyboard_check(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    if (gKeyboardOverride != nullptr &&
        gKeyboardOverride(ctx->currentInstance, key, KEYBOARD_QUERY_CHECK, &overridden)) {
        return RValue_makeBool(overridden);
    }
...
static RValue builtin_keyboard_check_pressed(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    bool overridden = false;
    if (gKeyboardOverride != nullptr &&
        gKeyboardOverride(ctx->currentInstance, key, KEYBOARD_QUERY_PRESSED, &overridden)) {
        return RValue_makeBool(overridden);
    }
...
static RValue builtin_keyboard_check_released(VMContext* ctx, RValue* args, int32_t argCount) {
    ...
    bool overridden = false;
    if (gKeyboardOverride != nullptr &&
        gKeyboardOverride(ctx->currentInstance, key, KEYBOARD_QUERY_RELEASED, &overridden)) {
        return RValue_makeBool(overridden);
    }
